...
- Los uids de escena escritos a mano en los `.tscn` (`uid://hpreferencias001`, `uid://bhistorial0001`, `uid://bgestoritem001/002`) no estaban registrados en `uid_cache.bin`, y `uid://cxtkcrqj7ne7i`/`cup4r7bqx0age` tienen 13 chars → desbordan int64 (uids inválidos). Al exportar (el pack convierte las escenas a `.scn` y resuelve por uid) `Main.tscn` fallaba al instanciar `VentanaAgregar`/`VentanaPreferencias` (`Cannot get class ''` → placeholder). Arreglo: referenciar las subescenas SOLO por ruta en `Main.tscn` (sin `uid=`), como hace `Dashboard.tscn`. Verificación: `--export-pack "Windows" build/x.pck` + `godot --headless --main-pack x.pck` (imitando el export del editor).
